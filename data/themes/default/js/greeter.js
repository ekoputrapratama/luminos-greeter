...
function defaultBG() {
  ...
  $(".bg")
    .fadeTo("slow", 0.3, function() {
      $(".bg").css(
        Object.assign(config.styles.background, {
...
$(document).ready(function() {
  function buildUserList() {
    ...
    for (var i in lightdm.users) {
      ...
      var item =
        ...
        '  <div href="#' +
        user.name +
        '" class="item" onclick="authenticate(event,\'' +
        user.name +
        '\')" data-session="' +
        lastSession +
        '">' +
        ...
        "    </div>" +
        '    <div class="info">' +
        '      <p role="presentation" class="wpW1cb">' +
        user.display_name +
        "</p>" +
        '      <p class="uRhzae" role="heading" aria-level="2">Last Session ' +
        lastSession +
        "</p>" +
        "     </div>" +
        "  </div>" +
  ...
  function buildSessionList() {
    ...
    for (var i in lightdm.sessions) {
      ...
      var button =
        ...
        session.key +
        '" onclick="sessionToggle(this)" class="' +
        theClass +
        '">' +
        session.name +
        "</a></li>";
  ...
  fetch("config.json").then(async function(res) {
    ...
    for (let i in config.styles["actionButton:hover"]) {
      style +=
        ...
        i.toLowerCase() +
        ":" +
        config.styles["actionButton:hover"][i] +
        ";";
    }
    $(".actionButton").attr("style", style);
    $(".other-account").attr("style", style);
  ...
  $(window).load(function() {
     ...
     */
    if (
      localStorage.getItem("bgdefault") === null &&
      localStorage.getItem("bgsaved") === null
    ) {
      localStorage.setItem("bgdefault", "1");
    }
    if (
      localStorage.getItem("bgsaved") !== null &&
      localStorage.getItem("bgdefault") === "0"
    ) {
      $(".bg")
        .fadeTo("slow", 0.3, function() {
          $(".bg").css(
            Object.assign(
              config && config.styles ? config.styles.background : {},
              {
    ...
    if (eval("lightdm.can_" + id)) {
      ...
      $("#actionsArea").append(
        '\n<button type="button" class="btn btn-default ' +
          id +
          ' actionButton" data-toggle="tooltip" data-placement="top" title="' +
          label +
          '" data-container="body" onclick="handleAction(\'' +
          id +
          '\')"><i class="fa fa-' +
          id2 +
          '"></i></button>'
